...
public class TaskSerializer implements Serializer {
    ...
    public String serialize(Entity e){
        if(e instanceof Task){
            Task task = (Task) e;
            return "Task" + "/" + task.id + "/" + task.getTitle() + "/" + task.getDescription() + "/" + task.getDueDate() + "/" + task.getStatus() + "/" + task.getType() + "/" + task.getCreationDate() + "/" + task.getLastModificationDate();
        }
    ...
    public Entity deserialize(String s){
            ...
            task.setCreationDate(LocalDate.parse(split[7]));
            task.setLastModificationDate(LocalDate.parse(split[8]));
            return task;
    }
}
